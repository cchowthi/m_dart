...
def load(   dsnin=None,             #Dataframe to load            
            ...
            issubjectlisting=True,  #Subject related listing
            ...
            dev=True                #load to prod, dev, or xlsx sheet (dev = devtest=Y, xlsx = devtest=UAT)
            ):
    # =============================================================================
    # Load Parameters 
    # =============================================================================           
    ...
    if not args.issubjectlisting:
        if any(x in args.keyvars for x in ['SCRNID','SCRNNUM','SUBJID','SUBJECT_ID']):
    ...
    if 'g_testing' in init:
        if not args.loadtype:
            ...
            if init['g_testing'] == 3:
                ...
                sys.exit(0)
        if (not args.loadtype or args.loadtype == 'UAT') and init['g_testing'] == 4:
            curtim = datetime.datetime.now().strftime("%Y%m%d_%H:%M:%S")
            ds.to_pickle("__dart_.pkl")
            sys.exit(0)     
    print()
    ...
    print('\t' + bcolors.HEADER + 'rowid:' + bcolors.ENDC + ':' + bcolors.UNDERLINE + init['_rowid'] + bcolors.ENDC)
    print('\t' + bcolors.HEADER + 'gildno:' + bcolors.ENDC + ':' + bcolors.UNDERLINE + init['_GILDNO'] + bcolors.ENDC)
    print()
